	...
	<view>
		...
		<view class="goods-info-wrap">
			...
			<view class="recommend">
				<uni-fav :checked="isFav" class="favBtn" circle="true" bgColor="#fff" bgColorChecked="#dd524d"
					@click="favClick" />
				<text>发货：湖南长沙</text>
				<text>快递：免运费</text>
			</view>
		...
		<view class="goods-nav-btn">
			<uni-goods-nav :fill="true" :options="options" @click="clickLeftOptions" :buttonGroup="buttonGroup"
	...
	export default {
		...
		methods: {
			...
			previewImg(i) {
				uni.previewImage({
					...
					urls: this.goodsInfo.pictures_big //预览地址
				})
			},
			favClick() {
		...
		watch: {
			totalGoods: {
				...
				immediate: true //当前的监听的方法,会在开始监听之后先调用1次
			},
		},
		computed: {
