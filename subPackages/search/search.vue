<template>
	<view>
		<view class="search-wrap">
			<uni-search-bar @input="input" placeholder="搜索" radius="20" cancelButton="none" bgColor="#fff">
			</uni-search-bar>
		</view>
		<!-- 搜索历史 -->
		<view class="search-history-wrap" v-if="searchResult.length === 0">
			...
			<view class="search-history-list">
				<uni-tag :text="item" type="default" v-for="(item,i) in history" :key="i" @click="goGoodsList(item)">
				</uni-tag>
			</view>
		</view>
		<!-- 搜索结果列表 -->
		<view class="search-result-list" v-else>
			<view class="search-result-item" v-for="(item, i) in searchResult" :key="i"
	...
	export default {
		...
		methods: {
			...
			async getSearchDataList() {
				const {
					data: res
				} = await uni.$http.get("/api/search/qs", {
